...
- **Transparent bar** (kill the black strip over apps). The bar bg is **not** AOSP's
  ...
  but that's the app's own top region, not the bar.
...
> Apps whose window background is the app surface blend right in (BT: `windowBackground=@drawable/bt_bk`). Apps
...
> top, so fully blending it there needs a code change to the radio's root. Low priority, since `radio_bk` is already
...
- **Active tile circle**: **fixed by recoloring baked PNGs** (no smali, no framework, no bootloop). The active
  ...
  **cleared** for the active state, because SYU bakes the `#41A4F7` blue straight into per‑tile "pressed" PNGs:
  ...
  code (the `getCircleColor` call has no `move-result`). That was the red herring; the color lives in the assets, not
...
- **Tile labels**: shortened the long ones so they stop ellipsizing: `airplane_mode` → **Flight**,
  ...
  tile just kills the screen while audio keeps playing, so "Sleep" fits better than "Off Screen"). The wifi tile
...
RecentsActivity to apply the pack; neither is clean. Left as‑is.
