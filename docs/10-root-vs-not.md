...
| **Dark mode** | `adb shell cmd uimode night yes` (or `settings put secure ui_night_mode 2`) | Uses WRITE_SECURE_SETTINGS, which the adb shell already holds, so no root. Persists across reboots. |
